...
abstract class AbstractTaskTests : AbstractFileTests() {
    ...
    protected fun sortTimes(sortTimes: (String, String) -> Unit) {
        ...
        try {
            assertThrows(IllegalArgumentException::class.java) { sortTimes("input/1My_time_in.txt", "temp.txt") }
        } finally {
            File("temp.txt").delete()
        }
    }
    ...
    protected fun sortTemperatures(sortTemperatures: (String, String) -> Unit) {
        ...
        try {
            sortTemperatures("input/1My_temp_in.txt", "temp.txt")
            assertFileContent(
                "temp.txt",
                """
                    -55.3
    ...
    protected fun sortSequence(sortSequence: (String, String) -> Unit) {
        ...
        try {
            sortSequence("input/1My_seq_in.txt", "temp.txt")
            assertFileContent(
                "temp.txt",
                """
                        9
